...
class PyClosureLL(PyFunctionLL):
	...
	def declare_function_object(self, docstring):
		...
		self.v.ctx.add(c.Assignment('=', c.ID(stack_name), c.FuncCall(c.ID('MelanoStack_Create'), c.ExprList(
		...
		if self.own_scope_offset > 0 and '__stack__' not in self.v.ctx.names:
			...
			self.v.ctx.add(c.Assignment('=', c.ID(parent_stack), c.FuncCall(c.ID('PyMelanoFunction_GetStack'), c.ExprList(c.ID(parent_scope.ll.c_obj.name)))))
			c_parent_stack = c.ID(parent_stack)
		else:
			c_parent_stack = c.ID('__stack__')

		for i in range(self.own_scope_offset):
			self.v.ctx.add(c.FuncCall(c.ID('MelanoStack_SetLocals'), c.ExprList(
																		c.ID(stack_name),
																		c.Constant('integer', i),
																		c.ArrayRef(c_parent_stack, c.Constant('integer', i)))))

		# set the locals on the new function object
		self.v.ctx.add(c.FuncCall(c.ID('PyMelanoFunction_SetStack'), c.ExprList(
	...
	def maybe_recursive_call(self):
		yield
		self.v.ctx.add(c.FuncCall(c.ID('MelanoStack_RestoreLocals'), c.ExprList(
	...
	def runner_intro(self):
		...
		self.v.ctx.add(c.Assignment('=', c.ID(self.locals_name), c.FuncCall(c.ID('MelanoLocals_Create'), c.ExprList(c.Constant('integer', len(self.local_syms))))))
	...
	def runner_outro(self):
		...
		self.v.ctx.block_items.insert(-1, c.FuncCall(c.ID('MelanoLocals_Destroy'), c.ExprList(
